...
# 0010 — rackmarshal-cli

- **Status:** Draft
- **Owner:** Nathan Klick
- **Date:** 2026-09-15
- **Summary:** `rackmarshal-cli` is the operator CLI, a cobra command tree over `rackmarshal-sdk`. Each profile pins an
  environment ID and CA bundle and verifies the gateway before any credential is sent. Login uses the
  OAuth 2.0 device authorization grant against `rackmarshal-sso`, subject to 0007. Credentials live in the OS
...
0001 defines `rackmarshal-cli` as the operator CLI that talks to the gateway through `rackmarshal-sdk`
...
0003 leaves interactive login and profile files to this document and [0007](0007-rackmarshal-sso.md).
...
- Implementing API clients, TLS, or revocation — `rackmarshal-sdk` ([0003](0003-rackmarshal-sdk.md)).
- Deciding the login protocol and token lifetimes — [0007](0007-rackmarshal-sso.md) and
  [0006](0006-rackmarshal-identity.md).
- Agent enrollment on hosts — `rackmarshal-agent` ([0012](0012-rackmarshal-agent.md)).
...
- **Resource commands** — thin wrappers over `rackmarshal-sdk` operations with consistent flags and output.
...
The binary is `rackmarshal-cli`, per CONVENTIONS. Commands are noun-then-verb:
...
├── agent             list | get ID | revoke ID      (rackmarshal-identity)
├── api-token         create | list | revoke ID      (rackmarshal-identity)
├── audit             verify [--since TIME]          (rackmarshal-identity)
...
- **Global flags** — `--profile`, `-o/--output table|json|yaml`, `--no-color`, `--quiet`, `--timeout`,
  `--log-level`, and `--yes`.
- **Lists** follow `nextCursor` automatically through `rackmarshal-sdk`'s `pagination.All`; `--limit` caps the
  total.
- **`rackmarshal-sdk` only.** API calls go only through `rackmarshal-sdk`'s generated clients. The single exception is
...
- **`audit verify`** — reads the signed anchors from the store named by `audit.anchor.uri` and the event
  chain from `rackmarshal-identity`, recomputes the hashes, and reports the first sequence where they diverge
  or an interval with no anchor. It needs read access to the anchor account only, never the database, so
  the check does not depend on the service being honest ([0006](0006-rackmarshal-identity.md)).
- **Later commands** — provisioner commands arrive with [0011](0011-rackmarshal-provisioner.md).
...
- **`profile verify`** runs automatically on `add`. It connects through `rackmarshal-sdk` `client.New` with
  `WithEnvironment(id, roots)`, which requires the chain to reach the pinned roots and the certificate to
  carry `spiffe://<id>/service/rackmarshal-gateway`. It then calls `GET /gateway/v1alpha1/environment` without
  credentials ([0008](0008-rackmarshal-gateway.md)) and records `name` and `tier`. A later name or tier mismatch
...
Proposed for [0007](0007-rackmarshal-sso.md) to accept or replace: **the OAuth 2.0 device authorization grant**
([RFC 8628](https://www.rfc-editor.org/rfc/rfc8628)), with `rackmarshal-cli` as a public client.
...
1. After `profile verify`, the CLI POSTs to `/identity/oidc/<environment-id>/device-authorization` on the
   ...
   proxies it to `rackmarshal-identity`, which owns the protocol endpoints (0006).
2. It prints the `verification_uri` and `user_code` to stderr and opens a browser unless `--no-browser` is
   set or no display is available. The user confirms the environment name that `rackmarshal-sso` shows next to
...
which requires `rackmarshal-sdk` to expose that client. Refresh tokens should rotate on use, as
...
Entries are keyed `rackmarshal-cli/<environment-id>/<profile>`. Before attaching a token, the CLI checks that
...
- **No silent fallback.** Unlike `gh`, which falls back to a plaintext file when no credential store is
  found, `rackmarshal-cli` fails with an explanation. The operator must choose `--credential-store file`, which
...
```
$ rackmarshal-cli enrollment-token create --tenant acme --label site=dc1 --ttl 4h --output-file token.txt
...
- **Validation** — `--ttl` defaults to `1h`, and the CLI rejects anything over `24h` before calling
  `rackmarshal-identity`, which remains authoritative. `--count N` (at most 100) creates N tokens, one per host.
...
- **Rackmarshal** — `rackmarshal-sdk` (clients, `problem`, `pagination`), `rackmarshal-common` (`logging`, `environment`,
  `telemetry`), and `rackmarshal-api-schema` models, through the SDK.
...
- **Removed from the starter** — `internal/database` (pgx, bun, goose) and `internal/pool`
  (`panjf2000/ants/v2`).
- **Measured footprint** — the proposed set plus `rackmarshal-common`'s OpenTelemetry stack links 23 third-party
...
- **Verify before credentials.** Every command that uses credentials first builds the pinned
  `rackmarshal-sdk` client. No flag skips verification, and there is no `--insecure`.
...
- **Supply chain** — releases ship cosign-signed binaries and the starters' signed CycloneDX SBOMs.
  `version --verbose` prints the module version and the `rackmarshal-sdk` schema version.
...
- **Telemetry export** is off by default (`RACKMARSHAL_CLI_TELEMETRY_ENABLED=false`); a CLI should not phone home.
  Trace context still propagates: `rackmarshal-common`'s `WrapTransport` adds `traceparent`, and errors print the
  server's `traceId`.
...
- **Environment pinning** — against `rackmarshal-sdk` `sdktest` gateways: wrong CA, wrong trust domain, wrong
...
- **A hand-written RFC 8628 client** — zero modules instead of one, but more security-sensitive code to
  maintain.
- **Device flow in `rackmarshal-sdk` `pkg/auth`** — reusable by third parties, but 0003 places interactive login
  out of scope.
...
- **Login protocol** — device grant, loopback PKCE, or both? Are token endpoints behind the gateway's
  `/sso/` routes ([0007](0007-rackmarshal-sso.md), [0008](0008-rackmarshal-gateway.md))?
- **SDK surface** — can `rackmarshal-sdk` expose its pinned `*http.Client` and a `--ca-sha256` bootstrap helper
  shared with `enroll`?
...
- **Binary name** — `rackmarshal-cli` per convention, or an official `rackmarshal` alias?
...
- [0003 — rackmarshal-sdk](0003-rackmarshal-sdk.md), [0004 — rackmarshal-common](0004-rackmarshal-common.md),
  [0008 — rackmarshal-gateway](0008-rackmarshal-gateway.md), [0009 — rackmarshal-inventory](0009-rackmarshal-inventory.md),

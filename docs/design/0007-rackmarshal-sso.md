...
# 0007 — rackmarshal-sso

- **Status:** Draft
- **Owner:** Nathan Klick
- **Date:** 2026-09-15
- **Summary:** `rackmarshal-sso` is the internet-facing login site and federation broker. It renders the login,
  tenant selection, and device verification pages for `rackmarshal-identity`'s login challenges, and
  authenticates users with local credentials or through OIDC and SAML exchanges with external IdPs,
  always resolving to a `rackmarshal-identity` principal. It keeps no database, and `rackmarshal-cli` logs in with
...
0001 makes `rackmarshal-sso` the federation broker that fronts login and hosts the login site, authenticating
against the internal `rackmarshal-identity` IdP or an external SAML/OIDC IdP, with either path resolving to a
`rackmarshal-identity` principal ([Auth split](0001-project-repositories.md#auth-split)). It exists as a
...
[0006](0006-rackmarshal-identity.md) proposes the login-challenge model this document implements.
...
- A `rackmarshal-cli` login that never handles the user's password and pins the environment.
...
- Operator administration pages — [0018](0018-rackmarshal-console.md) owns those, and
  [0016](0016-web-ui-architecture.md) the stack they share with this site.
- CLI profile storage — [0010](0010-rackmarshal-cli.md).
...
- Accept or reject `rackmarshal-identity` login challenges and device approvals.
...
JSON API, so they have no OpenAPI document (0002: `rackmarshal-sso` adds documents only for JSON APIs).

| Route                                     | Purpose                                                         |
|-------------------------------------------|-----------------------------------------------------------------|
| `GET /login?login_challenge=`             | Choose a method: local account or a tenant's IdP                |
| `POST /login/password`, `/login/webauthn` | Local credentials, verified by `rackmarshal-identity`                 |
...
| `POST /logout`                            | End the session at `rackmarshal-identity` and clear cookies           |

Health probes stay at `/livez`, `/readyz`, and `/healthz` (CONVENTIONS) on a separate private port.

#### Internal calls

`rackmarshal-sso` calls `rackmarshal-identity`'s `internal` operations (0006) over TLS 1.3 mutual TLS with its
`spiffe://<environment-id>/service/rackmarshal-sso` certificate, using the `rackmarshal-sdk` identity client:
...
- **OIDC RP** — [`coreos/go-oidc/v3`](https://github.com/coreos/go-oidc) with
  ...
  authenticate with `private_key_jwt` where supported, otherwise a client secret from `rackmarshal-identity`.
...
- **Principal resolution** — by the pair of provider ID and the IdP's stable subject (`sub` or a
  ...
  Group-to-role mappings are applied by `rackmarshal-identity`.

#### `rackmarshal-cli` login: device authorization grant

Proposed: [RFC 8628](https://www.rfc-editor.org/rfc/rfc8628), which fits CLIs on remote hosts without a
browser.

```
rackmarshal-cli ──► gateway ──► identity POST /identity/oidc/<env-id>/device-authorization
              {client_id: rackmarshal-cli, scope}      ◄── device_code, user_code, verification_uri, interval
rackmarshal-cli prints: "Open https://login.qa-east.example.net/device and enter BCDF-GHJK"
...
rackmarshal-cli ──► gateway ──► identity POST /token (grant_type=device_code), every `interval` seconds
                                                  ◄── access token (10m), refresh token (rotating)
```

- **Environment pinning** — `rackmarshal-cli` sends the device request only after `rackmarshal-sdk` has verified
...
- **User codes** — 8 characters from RFC 8628's 20-letter consonant alphabet, input accepted in any
  case and with or without the hyphen. Attempts are rate-limited in `rackmarshal-identity`, following
  RFC 8628 §5.1.
...
- **WebAuthn** — a hand-written script of about 100 lines calling `navigator.credentials.get`; the
  server-side check is in `rackmarshal-identity`.
- **Rackmarshal** — `rackmarshal-sdk` (identity client, `pkg/tlsconfig`), `rackmarshal-common`.

### Data & storage

No database. State lives in three places:

- **`rackmarshal-identity`** — sessions, login challenges, federated links, IdP configuration, and attempt
  counters.
- **Flow cookie** `__Host-rackmarshal-flow` — AES-256-GCM sealed `{challengeId, providerId, state, nonce,
  ...
  associated data.
- **Session cookie** `__Host-rackmarshal-session` — an opaque reference to the `rackmarshal-identity` session,
...
- Per-account and per-user-code attempt counters live in `rackmarshal-identity`, so limits hold across
  `rackmarshal-sso` instances.
- Errors never reveal whether an account exists, and login pages avoid CAPTCHAs (see Accessibility).

#### Other controls

- Redirects go only to URLs returned by `rackmarshal-identity`, which matches registered redirect URIs
  exactly; `rackmarshal-sso` has no open redirect parameter.
...
- `rackmarshal-sso` holds no token-signing keys. Tokens always come from `rackmarshal-identity` (0006); the only
...
- Login outcomes are logged with `rackmarshal.sso.method` (`password`, `webauthn`, `oidc`, `saml`,
  `device`), `rackmarshal.sso.provider.id`, and `rackmarshal.tenant.id`; `rackmarshal-identity` records the audit event.
...
- **Loopback redirect with PKCE for `rackmarshal-cli`** ([RFC 8252](https://www.rfc-editor.org/rfc/rfc8252)) —
  smoother on desktops, but fails on remote hosts. Could be added beside the device grant.
- **Resource owner password grant in `rackmarshal-cli`** — the CLI would handle passwords, and RFC 9700
  forbids it.
- **`rackmarshal-sso` as the OIDC provider itself** — puts protocol logic and signing keys on the exposed
...
- **Server-side flow state in PostgreSQL** — survives cookie loss, but gives `rackmarshal-sso` a database to
  harden.
...
- **Internal SDK clients** — this design needs `rackmarshal-sdk` clients for `internal` operations, which
  0003 leaves open.
...
- [0001 — Project Repositories](0001-project-repositories.md) — auth split, environment identity,
  environment awareness.
- [0003 — rackmarshal-sdk](0003-rackmarshal-sdk.md), [0006 — rackmarshal-identity](0006-rackmarshal-identity.md),

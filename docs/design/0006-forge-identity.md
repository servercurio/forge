...
0001 assigns `forge-identity` the internal SAML/OIDC IdP, accounts, API tokens, RBAC, tenancy, and
...
- Login pages, external IdP federation, and cookies — [0007](0007-forge-sso.md).
- Request authentication at the edge and routing — [0008](0008-forge-gateway.md).
- The offline root and environment creation — [0005](0005-forge-infrastructure.md).
- Key storage on agents — [0012](0012-forge-agent.md).
...
- **`forge-gateway`** forwards `operator` and `agent` operations and the public protocol endpoints.
- **`forge-sso`** and other services call `internal` operations directly with service certificates.
- **Service enrollment** is the one route that accepts a connection without a client certificate
  (`security: []`), because the enrolling service and `forge-gateway` have no certificate yet. This
...
`forge-sso` with `login_challenge=<id>`. `forge-sso` authenticates the user, then accepts the challenge
over mutual TLS, and the browser returns to `/authorize` to receive a code. `forge-identity` keeps all
protocol logic; `forge-sso` keeps all pages and federation.
...
| `POST /identity/v1alpha1/device-approvals`                    | internal   | mTLS, `forge-sso` only |
| `POST /identity/v1alpha1/token-introspections`                | internal   | mTLS, `forge-gateway` only |
...
- **Access tokens** — JWT per [RFC 9068](https://www.rfc-editor.org/rfc/rfc9068), ES256, 10 minutes.
  `iss` is the issuer URL above; `aud` is `spiffe://<environment-id>/service/forge-gateway`. Claims:
...
two-thirds of the lifetime. On startup `forge-identity` issues its own service certificate from the
...
- **Forge** — `forge-api-schema`, `forge-sdk` (`pkg/spiffe`, `pkg/tlsconfig`, `pkg/revocation`),
  `forge-common`.
...
Prefix `FORGE_IDENTITY_`, plus the starter's `server` and `database` blocks and CONVENTIONS' `logging`,
`telemetry`, and `environment` blocks. The DSN moves to `database.dsnFile`.

| YAML                               | Variable                                    | Default                  |
|------------------------------------|---------------------------------------------|--------------------------|
| `issuer.baseUrl`                   | `FORGE_IDENTITY_ISSUER_BASE_URL`            | none — required          |
| `keys.backend`                     | `FORGE_IDENTITY_KEYS_BACKEND`               | `pkcs11`                 |
...
- **Hosting login pages in `forge-identity`** — fewer hops, but puts an internet-facing surface on the
...
- **Gateway exceptions** — `/authorize`, `/token`, `/device-authorization`, JWKS, and agent enrollment
  must pass the gateway unauthenticated. Agree the allowlist with [0008](0008-forge-gateway.md).
...
- [0002](0002-forge-api-schema.md), [0003](0003-forge-sdk.md), [0004](0004-forge-common.md),
  [0005](0005-forge-infrastructure.md), [0007](0007-forge-sso.md), [CONVENTIONS.md](CONVENTIONS.md).

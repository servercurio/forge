...
0001 defines `rackmarshal-sdk` as the generated Go client for the public API, used by `rackmarshal-cli`,
`rackmarshal-agent`, and third parties ([Repository inventory](0001-project-repositories.md#repository-inventory)),
published as one module with one version (Resolved: *SDK modularity*). All SDK traffic enters through
`rackmarshal-gateway` ([Architecture at a glance](0001-project-repositories.md#architecture-at-a-glance)).
...
- Server-side request handling, routing, and authorization — services and [0008](0008-rackmarshal-gateway.md).
- TPM and OS keystore integrations — the SDK defines the interface, [0012](0012-rackmarshal-agent.md)
  implements them.
- Interactive login flows and CLI profile files — [0007](0007-rackmarshal-sso.md) and
  [0010](0010-rackmarshal-cli.md).
- Token formats, CA operation, and issuance policy — [0006](0006-rackmarshal-identity.md).

## Proposal

### Responsibilities

- **API clients** — generated per service and API version from `rackmarshal-api-schema`.
...
└── examples/                    # nested module: examples wired to rackmarshal-common
```
...
c, err := client.New(
    ...
    client.WithTransportWrapper(telemetry.WrapTransport),    // e.g. from rackmarshal-common; optional
)
...
imports only the standard library, `rackmarshal-api-schema`, and the SDK itself. The cost is keeping the
...
- **Headers** — every request carries a fresh `X-Request-Id` and `User-Agent: rackmarshal-sdk/<version>`;
...
- **`revocation`** — `Checker` implements 0001's rules. It asks the certificate's OCSP responder through
  ...
  `rackmarshal-gateway` uses it for agents, and every service uses it for peer services.
- **`enroll`** — the CSR flow from 0001, shared by agents and services. A `Credential` names the
  ...
  account token (services only, [0005](0005-rackmarshal-infrastructure.md)). Both read the file when used,
  ...
  1. `ParseToken` extracts the environment ID and CA certificate hash without contacting the server. The
     wire format comes from [0006](0006-rackmarshal-identity.md). A service account token carries neither, so
  ...
  4. `Result` returns the certificate and chain, the CA bundle, and the environment ID, name, and tier,
     which `rackmarshal-agent` records (0001).
...
- **`github.com/servercurio/rackmarshal-api-schema`** — models and embedded documents.
- **`golang.org/x/crypto`** — `ocsp` in `pkg/revocation` only.
- **Nothing else in the root module.** No `rackmarshal-common`, zerolog, OpenTelemetry, gRPC, `go-spiffe` (whose
...
- **Nested modules** — `conformance` uses kin-openapi v0.149.0, which brings `go-openapi/jsonpointer`,
  ...
  `rackmarshal-common`.
- **Consumers** — `rackmarshal-cli`, `rackmarshal-agent`, `rackmarshal-gateway`, and every service (for identity packages
...
- **Environment pin required.** `client.New` fails without `WithEnvironment`. The gateway certificate
  must chain to the pinned roots and carry `spiffe://<environment-id>/service/rackmarshal-gateway` before any
  token is sent, which implements 0001's `rackmarshal-cli` rule for every SDK user.
...
the environment's name, tier, ID, and CA bundle, typed with the tier enum from `rackmarshal-api-schema`'s
`common/v1`. Hosts decide how the tier changes behavior, through `rackmarshal-common`
([Environment awareness](0001-project-repositories.md#environment-awareness)).

### Logging & telemetry

The SDK does not log or import OpenTelemetry. Callers pass `WithTransportWrapper`; `rackmarshal-cli`,
`rackmarshal-agent`, and services pass `rackmarshal-common`'s instrumented transport, which adds W3C `traceparent` and
client spans ([0004](0004-rackmarshal-common.md)). `*problem.Error.TraceID` carries the server's trace ID into
...
- **Regeneration** — a `rackmarshal-api-schema` release triggers a 100-series workflow here that updates the
  dependency, runs `task generate`, and opens a pull request titled `feat: regenerate for rackmarshal-api-schema
  vX.Y.Z`.
- **Versioning** — one module and one version (0001), `v0.x` until accepted. Each release notes the
  `rackmarshal-api-schema` version it was generated from, and `client.SchemaVersion` exposes it.
- **Compatibility** — adding an API version is a minor release. Removing a package follows the API's
  stage rules in [0002](0002-rackmarshal-api-schema.md).
...
- **Identity packages in `rackmarshal-common`** — keeps `rackmarshal-sdk` API-only, but every third-party SDK user
  would pull zerolog and OpenTelemetry through `rackmarshal-common`, and `rackmarshal-common` would mix security
  with observability.
- **SDK depends on `rackmarshal-common`** for logging and tracing — consistent wiring, but imposes Rackmarshal's
  logging stack on third parties.

## Open questions

- **`rackmarshal-common` in the SDK** — 0001 says `rackmarshal-common` is used by every Rackmarshal Go repository. This
  draft keeps it out of the SDK's root module (injection plus the `examples` module). Does that meet the
  intent?
- **Service enrollment endpoint** — proposed directly on `rackmarshal-identity`, because `rackmarshal-gateway` needs
  its own certificate first. Or should it go through the gateway like agents? To settle with
  [0006](0006-rackmarshal-identity.md) and [0008](0008-rackmarshal-gateway.md).
...
- [0002 — rackmarshal-api-schema](0002-rackmarshal-api-schema.md) — contract, extensions, versioning.
